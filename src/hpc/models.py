...
class SparseHopfieldNetwork:
    ...
    def __init__(self, patterns_dict, p=None, transform=True):
        ...
        # =================================================================
        # =================== SOLUTION ==============================
        self.p = p if p is not None else self.flattened_patterns.sum(axis=1).mean()/self.N_neurons  # sparsity
        self.w = (self.flattened_patterns - self.p).T @ (self.flattened_patterns - self.p)/self.N_neurons
        self.w = self.w*(1-np.eye(self.N_neurons))  # zero diagonal
        # ===========================================================*
        self.theta_opt = (self.N_neurons - 1)/self.N_neurons*(self.p**3 - 3/2*self.p**2 + self.p/2)  # threshold
        self.theta = self.theta_opt*np.ones(self.N_neurons)
        if self.transform:
            self.theta_rand = self.p*self.w.sum(axis=1)
            self.theta += self.theta_rand

        self.set_state(p=self.p)  # initialises the state of the network
        return

    # =================== INITIALISE AND UPDATE NETWORK STATE  ======================
    def set_state(self, state=None, p=None):
        """Sets the state of the Hopfield network. If p != None, sets state to a random vector with sparsity p"""
        if p is not None:
            self.state = np.random.choice(2, self.N_neurons, p=[1-p, p])
        else:
    ...
    def update_state(self, asynchronous=True):
        ...
        if asynchronous == True:
            ...
            self.state[i] = np.heaviside(self.w[i, :] @ self.state - self.theta[i], 0)  # update the neuron

        # synchronous updates all neurons at once
        elif asynchronous == False:
            self.state =  np.heaviside(self.w @ self.state - self.theta, 0)
    ...
    def get_energy(self, state=None):
        ...
        state = self.state if state is None else state
        return -0.5 * state @ self.w @ state + np.dot(self.theta, state)
